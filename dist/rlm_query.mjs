...
import { chmodSync as chmodSync3, closeSync as closeSync2, existsSync as existsSync3, mkdirSync as mkdirSync2, mkdtempSync, openSync as openSync2 } from "node:fs";
...
function ensureRuntimeStatePaths() {
  ...
  if (!missing)
    return;
  const stateRoot = mkdtempSync(path4.join(tmpdir(), `ypi_runtime_${process.env.RLM_TRACE_ID}_`));
  chmodSync3(stateRoot, 448);
  process.env.RLM_CALL_COUNTER_FILE ||= path4.join(stateRoot, "calls.counter");
  process.env.RLM_CONCURRENCY_DIR ||= path4.join(stateRoot, "concurrency");
  process.env.PI_TRACE_FILE ||= path4.join(stateRoot, "trace.jsonl");
  process.env.RLM_COST_FILE ||= path4.join(stateRoot, "cost.jsonl");
}
...
import { chmodSync as chmodSync4, copyFileSync as copyFileSync2, existsSync as existsSync5, mkdtempSync as mkdtempSync2, readFileSync as readFileSync3, readdirSync as readdirSync2, rmSync as rmSync4, writeFileSync as writeFileSync3 } from "node:fs";
...
function createAsyncJob(input) {
  ...
  const jobDir = mkdtempSync2(path6.join(root, `rlm_async_${traceId}_`));
...
import { closeSync as closeSync3, existsSync as existsSync6, mkdtempSync as mkdtempSync3, openSync as openSync3, rmSync as rmSync5, writeSync } from "node:fs";
...
async function spoolStdin(options) {
  const directory = mkdtempSync3(path7.join(process.env.TMPDIR || tmpdir4(), "ypi_cli_stdin_"));
...
function openSecureDirectory(directoryPath) {
  ...
  const descriptor = openSync4(normalized, constants3.O_RDONLY | (constants3.O_DIRECTORY || 0) | (constants3.O_NOFOLLOW || 0));
  try {
...
function assertDirectoryIdentity(directory) {
  ...
  const pathMetadata = lstatSync2(directory.path, { bigint: true });
...
function openSecureForkSource(sourcePath) {
  if (!path10.isAbsolute(sourcePath)) {
    throw proofError(`RLM_REQUIRE_TRANSCRIPTS=1 requires an absolute fork source path: ${sourcePath}`);
  }
  const descriptor = openSync5(sourcePath, constants4.O_RDONLY | (constants4.O_NOFOLLOW || 0));
  try {
    const metadata = fstatSync2(descriptor, { bigint: true });
    const pathMetadata = lstatSync3(sourcePath, { bigint: true });
...
function createHeldTranscript(childSession, directory, baselineSource) {
  ...
  if (path10.dirname(childSession) !== expectedParent) {
    throw proofError("Required child transcript must be a direct child of the session directory.");
  }
  const temporary = path10.join(expectedParent, `.${path10.basename(childSession)}.${process.pid}.${randomBytes5(8).toString("hex")}.tmp`);
  let descriptor;
  try {
    descriptor = openSync5(temporary, constants4.O_CREAT | constants4.O_EXCL | constants4.O_RDWR | (constants4.O_NOFOLLOW || 0), PRIVATE_FILE_MODE2);
    ...
    fsyncSync2(directory.descriptor);
    const metadata = fstatSync2(descriptor, { bigint: true });
    const pathMetadata = lstatSync3(childSession, { bigint: true });
...
function prepareTranscriptProof(input) {
  ...
  if (!path10.isAbsolute(input.childSession)) {
    throw proofError(`RLM_REQUIRE_TRANSCRIPTS=1 requires an absolute child transcript path: ${input.childSession}`);
  }
  const childSession = path10.resolve(input.childSession);
  const directory = openSecureDirectory(path10.dirname(childSession));
  try {
...
function assertTranscriptIdentity(lease) {
  ...
  const descriptorMetadata = fstatSync2(lease.descriptor, { bigint: true });
  const pathMetadata = lstatSync3(lease.childSession, { bigint: true });
...
function finalizeTranscriptProof(lease, identity) {
  ...
  const receipt = {
    ...
    transcript_file: path10.basename(lease.childSession),
...
function normalizeRepositoryRelativePath(value, label) {
  ...
  if (path11.posix.isAbsolute(value)) {
...
function implementerRegistryPaths(commonGitDir) {
  const root = path12.join(commonGitDir, "ypi-implementers");
  return {
    root,
    leases: path12.join(root, "leases"),
    lock: path12.join(commonGitDir, "ypi-implementers.lock"),
    retired: path12.join(root, "retired"),
    staging: path12.join(root, "staging")
  };
}
...
function assertRegistryDirectory(directory) {
  const metadata = lstatSync4(directory);
  if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
...
function acquireRegistryLock2(commonGitDir, deadlineMilliseconds) {
  ...
  while (true) {
    ...
    try {
      ...
      created = true;
      atomicWriteJson(path13.join(paths.lock, "owner.json"), {
  ...
  return {
    ...
    release() {
      try {
        const owner = JSON.parse(readFileSync5(path13.join(paths.lock, "owner.json"), "utf8"));
        if (owner.token === token)
          rmSync6(paths.lock, { recursive: true, force: true });
      } catch {}
    }
...
function readImplementerLeaseRecords(commonGitDir) {
  ...
  if (unexpected) {
    throw new Error(`Implementer lease registry contains unexpected entry ${path13.join(paths.leases, unexpected.name)}. Run rlm_cleanup --repo <checkout> and inspect it before admitting more writers.`);
  }
  return entries.map((entry) => {
    const recordPath = path13.join(paths.leases, entry.name, "lease.json");
    try {
      const metadata = lstatSync4(recordPath);
      if (!metadata.isFile() || metadata.isSymbolicLink()) {
        throw new Error(`Implementer lease ${entry.name} metadata is not a regular file`);
      }
      return parseImplementerLeaseRecord(JSON.parse(readFileSync5(recordPath, "utf8")), entry.name, commonGitDir);
    } catch (error) {
...
function implementerLeaseDirectory(commonGitDir, token) {
  return path13.join(implementerRegistryPaths(commonGitDir).leases, token);
}
function writeImplementerLeaseRecord(record) {
  ...
  assertRegistryDirectory(directory);
  const target = path13.join(directory, "lease.json");
...
function createImplementerLeaseRecord(record, onStaged) {
  ...
  const stagingDirectory = path13.join(paths.staging, `.lease-${record.token}-${process.pid}-${randomBytes6(4).toString("hex")}.tmp`);
  mkdirSync4(stagingDirectory, { mode: 448 });
  try {
    atomicWriteJson(path13.join(stagingDirectory, "lease.json"), record);
    onStaged?.(record);
...
function removeImplementerLeaseRecord(commonGitDir, token) {
  ...
  const retired = path13.join(paths.retired, `.lease-${token}-${process.pid}-${randomBytes6(4).toString("hex")}.done`);
...
function parseGitlinks(indexEntries, root) {
  ...
  for (const record of indexEntries.split("\x00")) {
    ...
    const submoduleRoot = path14.join(root, ...gitPath2.split("/"));
    links.push({
      path: gitPath2,
      oid,
      initialized: existsSync8(path14.join(submoduleRoot, ".git"))
    });
  }
  return links;
}
function materializeBaselineIgnoreFiles(input, root, baselineHead, ignoreRoot) {
  ...
  for (const record of listing.split("\x00")) {
    ...
    if (gitPath2.split("/").at(-1) !== ".gitignore")
      continue;
    const destination = path14.join(ignoreRoot, ...gitPath2.split("/"));
    mkdirSync5(path14.dirname(destination), { recursive: true, mode: 448 });
...
function createConfinementState(input, root, baselineHead, leaseDirectory, scope) {
  const auditFile = path14.join(leaseDirectory, "writes");
  const baselineIgnoreRoot = path14.join(leaseDirectory, "baseline-ignore");
  const baselineIndexFile = path14.join(leaseDirectory, "baseline-index");
  const childPidFile = path14.join(leaseDirectory, "child-pid");
  const childReadyFile = path14.join(leaseDirectory, "child-ready");
  const scopeFile = path14.join(leaseDirectory, "scope");
  const submodulePathsFile = path14.join(leaseDirectory, "submodules");
...
function auditedPaths(confinement, root) {
  ...
  const paths = parseNulPaths(readFileSync6(confinement.auditFile, "utf8"));
  for (const relativePath of paths) {
    ...
    const absolutePath = path14.resolve(root, ...relativePath.split("/"));
...
function assertSubmodulesUnchanged(root, gitlinks) {
  for (const entry of gitlinks) {
    const submoduleRoot = path14.join(root, ...entry.path.split("/"));
    if (!entry.initialized) {
      if (existsSync8(path14.join(submoduleRoot, ".git"))) {
...
function bestEffortDiscardSetupWorktree(root, record) {
  if (record.worktreeContainer && record.worktreeRoot) {
    if (!path14.isAbsolute(record.worktreeContainer) || !path14.isAbsolute(record.worktreeRoot) || path14.basename(record.worktreeContainer) !== `ypi_ws_${record.token}` || record.worktreeRoot !== path14.join(record.worktreeContainer, "checkout")) {
      return "recorded setup workspace paths are invalid";
    }
  }
  if (record.worktreeContainer && existsSync8(record.worktreeContainer)) {
    try {
      const container = lstatSync5(record.worktreeContainer);
      const markerPath = path14.join(record.worktreeContainer, "owner");
      const marker = lstatSync5(markerPath);
      if (!container.isDirectory() || container.isSymbolicLink() || path14.basename(record.worktreeContainer) !== `ypi_ws_${record.token}` || record.worktreeRoot !== path14.join(record.worktreeContainer, "checkout") || !marker.isFile() || marker.isSymbolicLink() || readFileSync6(markerPath, "utf8").trim() !== record.token) {
      ...
      try {
        ...
        const marker = lstatSync5(markerPath);
        const actual = readFileSync6(markerPath);
...
function createGitWorktreeLease(input, root, scope) {
  ...
  try {
    assertAdmissibleCheckout(input, root);
    record.worktreeContainer = path14.join(tmpdir5(), `ypi_ws_${record.token}`);
    record.worktreeRoot = path14.join(record.worktreeContainer, "checkout");
    ...
    writeFileSync4(path14.join(record.worktreeContainer, "owner"), `${record.token}
`, { flag: "wx", mode: 384 });
    ...
    const snapshotIndexFile = path14.join(leaseDirectory, "snapshot-index");
    let finalized;
...
function createContextFile(input) {
  if (input.context !== undefined) {
    const contextPath = path15.join(mkdtempSync4(path15.join(tmpdir6(), "ypi_ctx_")), "context.txt");
...
function createPromptFile(prompt) {
  const promptPath = path15.join(mkdtempSync4(path15.join(tmpdir6(), "ypi_prompt_")), "prompt.txt");
...
function createStandaloneSystemPrompt(input, promptFile, contextFile) {
  if (!input.systemPromptPath || !existsSync9(input.systemPromptPath))
    return;
  const outputPath = path15.join(mkdtempSync4(path15.join(tmpdir6(), "ypi_system_")), "system-prompt.md");
  const section = renderActiveTaskFilesSection({
    contextPath: contextFile,
    promptPath: promptFile,
    rootPromptPath: input.rootPromptPath || promptFile
  });
  writeFileSync5(outputPath, `${readFileSync7(input.systemPromptPath, "utf8")}${section}`);
  return outputPath;
}
function childSessionFile(input) {
  ...
  if (transcriptsRequired()) {
    if (!path15.isAbsolute(sessionDir)) {
      throw new Error(`RLM_REQUIRE_TRANSCRIPTS=1 requires an absolute session directory: ${sessionDir}`);
    }
    return path15.join(sessionDir, `${safeTraceId(process.env.RLM_TRACE_ID || "ypi")}_d${input.childDepth}_c${input.callCount}.jsonl`);
  }
...
function removeArtifact(filePath) {
  if (filePath)
    rmSync8(path15.dirname(filePath), { recursive: true, force: true });
}
  ...
  return cleanupErrors([
    ...
    () => {
      if (input.isolatedPiRoot) {
        rmSync8(input.isolatedPiRoot, { recursive: true, force: true });
      }
    }
  ]);
}
...
function configuredParentAgentDir() {
  ...
  if (configured.startsWith("~/") || configured.startsWith("~\\")) {
    return path15.join(home, configured.slice(2));
  }
  return configured;
}
function projectSelectedProviderAuth(agentDir, selectedProvider) {
  ...
  const sourceAuthPath = path15.join(parentAgentDir, "auth.json");
  if (!existsSync9(sourceAuthPath))
    return;
  let parsed;
  try {
    parsed = JSON.parse(readFileSync7(sourceAuthPath, "utf8"));
  } catch {
  ...
  const targetAuthPath = path15.join(agentDir, "auth.json");
...
function acquireChildResources(input) {
  ...
  try {
    ...
    if (input.fullResourceIsolation) {
      isolatedPiRoot = mkdtempSync4(path15.join(tmpdir6(), "ypi_isolated_pi_"));
      chmodSync5(isolatedPiRoot, 448);
      const isolatedAgentDir = path15.join(isolatedPiRoot, "agent");
...
Inherited concurrency-slot resume also failed: ${resumeFailure.message}` : primaryMessage;
  ...
  try {
    ...
    if (resources.isolatedPiRoot) {
      env.PI_CODING_AGENT_DIR = path16.join(resources.isolatedPiRoot, "agent");
      env.PI_OFFLINE = "1";
    }
    ...
    const processResult = await runChildProcess({
      ...
      launchGate: {
        launcherPath: path16.join(runtime.root, "scripts", "launch-recursive-child.ts"),
...
var moduleExtensionPath = path17.basename(modulePath) === "rlm_query.mjs" && path17.basename(path17.dirname(modulePath)) === "dist" ? path17.join(path17.dirname(modulePath), "..", "extensions", "recursive.ts") : fileURLToPath2(new URL("../recursive.ts", import.meta.url));
    ...
    if (flags.async) {
      ...
      try {
        ...
        await writeStdout(`${JSON.stringify({
          job_id: path17.basename(path17.dirname(job.jobPath)),
...
var invokedPath = process.argv[1] ? path17.resolve(process.argv[1]) : "";
