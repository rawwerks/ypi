...
import { chmodSync as chmodSync3, copyFileSync as copyFileSync2, existsSync as existsSync8, mkdirSync as mkdirSync5, mkdtempSync as mkdtempSync3, readFileSync as readFileSync5, rmSync as rmSync7, writeFileSync as writeFileSync6 } from "node:fs";
...
function syncDirectory(directory) {
  const descriptor = openSync3(directory, constants2.O_RDONLY);
  try {
...
function atomicWriteFile(target, content, options = {}) {
  ...
  const temporary = path8.join(directory, `.${path8.basename(target)}.${process.pid}.${randomBytes3(4).toString("hex")}.tmp`);
  let descriptor;
  try {
    descriptor = openSync3(temporary, constants2.O_CREAT | constants2.O_EXCL | constants2.O_WRONLY, options.mode ?? 384);
...
function implementerRegistryPaths(commonGitDir) {
  const root = path9.join(commonGitDir, "ypi-implementers");
  return {
    root,
    leases: path9.join(root, "leases"),
    lock: path9.join(commonGitDir, "ypi-implementers.lock"),
    retired: path9.join(root, "retired"),
    staging: path9.join(root, "staging")
  };
}
...
function acquireRegistryLock(commonGitDir, deadlineMilliseconds) {
  ...
  while (true) {
    ...
    try {
      ...
      created = true;
      atomicWriteJson(path9.join(paths.lock, "owner.json"), {
      ...
      if (created) {
        rmSync5(paths.lock, { recursive: true, force: true });
        throw error;
      }
      if (error.code !== "EEXIST")
        throw error;
  ...
  return {
    ...
    release() {
      try {
        const owner = JSON.parse(readFileSync3(path9.join(paths.lock, "owner.json"), "utf8"));
...
function readImplementerLeaseRecords(commonGitDir) {
  ...
  if (unexpected) {
    throw new Error(`Implementer lease registry contains unexpected entry ${path9.join(paths.leases, unexpected.name)}. Run rlm_cleanup --repo <checkout> and inspect it before admitting more writers.`);
  }
  return entries.map((entry) => {
    const recordPath = path9.join(paths.leases, entry.name, "lease.json");
    try {
      const metadata = lstatSync(recordPath);
      if (!metadata.isFile() || metadata.isSymbolicLink()) {
        throw new Error(`Implementer lease ${entry.name} metadata is not a regular file`);
      }
      return parseImplementerLeaseRecord(JSON.parse(readFileSync3(recordPath, "utf8")), entry.name, commonGitDir);
    } catch (error) {
...
function implementerLeaseDirectory(commonGitDir, token) {
  return path9.join(implementerRegistryPaths(commonGitDir).leases, token);
}
function writeImplementerLeaseRecord(record) {
  ...
  assertRegistryDirectory(directory);
  const target = path9.join(directory, "lease.json");
...
function createImplementerLeaseRecord(record, onStaged) {
  ...
  const stagingDirectory = path9.join(paths.staging, `.lease-${record.token}-${process.pid}-${randomBytes4(4).toString("hex")}.tmp`);
  mkdirSync3(stagingDirectory, { mode: 448 });
  try {
    atomicWriteJson(path9.join(stagingDirectory, "lease.json"), record);
    onStaged?.(record);
...
function removeImplementerLeaseRecord(commonGitDir, token) {
  ...
  const retired = path9.join(paths.retired, `.lease-${token}-${process.pid}-${randomBytes4(4).toString("hex")}.done`);
...
function parseGitlinks(indexEntries, root) {
  ...
  for (const record of indexEntries.split("\x00")) {
    ...
    const submoduleRoot = path10.join(root, ...gitPath2.split("/"));
    links.push({
      path: gitPath2,
      oid,
      initialized: existsSync7(path10.join(submoduleRoot, ".git"))
    });
  }
  return links;
}
function materializeBaselineIgnoreFiles(input, root, baselineHead, ignoreRoot) {
  ...
  for (const record of listing.split("\x00")) {
    ...
    if (gitPath2.split("/").at(-1) !== ".gitignore")
      continue;
    const destination = path10.join(ignoreRoot, ...gitPath2.split("/"));
    mkdirSync4(path10.dirname(destination), { recursive: true, mode: 448 });
...
function createConfinementState(input, root, baselineHead, leaseDirectory, scope) {
  const auditFile = path10.join(leaseDirectory, "writes");
  const baselineIgnoreRoot = path10.join(leaseDirectory, "baseline-ignore");
  const baselineIndexFile = path10.join(leaseDirectory, "baseline-index");
  const childPidFile = path10.join(leaseDirectory, "child-pid");
  const childReadyFile = path10.join(leaseDirectory, "child-ready");
  const scopeFile = path10.join(leaseDirectory, "scope");
  const submodulePathsFile = path10.join(leaseDirectory, "submodules");
...
function auditedPaths(confinement, root) {
  ...
  for (const relativePath of paths) {
    ...
    const absolutePath = path10.resolve(root, ...relativePath.split("/"));
...
function assertSubmodulesUnchanged(root, gitlinks) {
  for (const entry of gitlinks) {
    const submoduleRoot = path10.join(root, ...entry.path.split("/"));
    if (!entry.initialized) {
      if (existsSync7(path10.join(submoduleRoot, ".git"))) {
...
function bestEffortDiscardSetupWorktree(root, record) {
  if (record.worktreeContainer && record.worktreeRoot) {
    if (!path10.isAbsolute(record.worktreeContainer) || !path10.isAbsolute(record.worktreeRoot) || path10.basename(record.worktreeContainer) !== `ypi_ws_${record.token}` || record.worktreeRoot !== path10.join(record.worktreeContainer, "checkout")) {
  ...
  if (record.worktreeContainer && existsSync7(record.worktreeContainer)) {
    try {
      ...
      const markerPath = path10.join(record.worktreeContainer, "owner");
      const marker = lstatSync2(markerPath);
      if (!container.isDirectory() || container.isSymbolicLink() || path10.basename(record.worktreeContainer) !== `ypi_ws_${record.token}` || record.worktreeRoot !== path10.join(record.worktreeContainer, "checkout") || !marker.isFile() || marker.isSymbolicLink() || readFileSync4(markerPath, "utf8").trim() !== record.token) {
      ...
      try {
        ...
        if (entries.length === 0) {
          rmSync6(record.worktreeContainer);
          return;
        }
        const markerPath = path10.join(record.worktreeContainer, "owner");
...
`);
  ...
  if (record.worktreeContainer && existsSync7(record.worktreeContainer)) {
    rmSync6(record.worktreeContainer, { recursive: true, force: true });
  }
  return;
}
function createGitWorktreeLease(input, root, scope) {
  ...
  try {
    assertAdmissibleCheckout(input, root);
    record.worktreeContainer = path10.join(tmpdir5(), `ypi_ws_${record.token}`);
    record.worktreeRoot = path10.join(record.worktreeContainer, "checkout");
    ...
    writeFileSync5(path10.join(record.worktreeContainer, "owner"), `${record.token}
`, { flag: "wx", mode: 384 });
    ...
    const snapshotIndexFile = path10.join(leaseDirectory, "snapshot-index");
    let finalized;
...
function createContextFile(input) {
  if (input.context !== undefined) {
    const contextPath = path11.join(mkdtempSync3(path11.join(tmpdir6(), "ypi_ctx_")), "context.txt");
...
function createPromptFile(prompt) {
  const promptPath = path11.join(mkdtempSync3(path11.join(tmpdir6(), "ypi_prompt_")), "prompt.txt");
...
function createStandaloneSystemPrompt(input, promptFile, contextFile) {
  if (!input.systemPromptPath || !existsSync8(input.systemPromptPath))
    return;
  const outputPath = path11.join(mkdtempSync3(path11.join(tmpdir6(), "ypi_system_")), "system-prompt.md");
...
function childSessionFile(input) {
  ...
  return path11.join(sessionDir, `${safeTraceId(process.env.RLM_TRACE_ID || "ypi")}_d${input.childDepth}_c${input.callCount}.jsonl`);
}
...
function removeArtifact(filePath) {
  if (filePath)
    rmSync7(path11.dirname(filePath), { recursive: true, force: true });
}
function configuredParentAgentDir() {
  ...
  if (configured.startsWith("~/") || configured.startsWith("~\\")) {
    return path11.join(home, configured.slice(2));
  }
  return configured;
}
function projectSelectedProviderAuth(agentDir, selectedProvider) {
  ...
  const sourceAuthPath = path11.join(parentAgentDir, "auth.json");
  ...
  const targetAuthPath = path11.join(agentDir, "auth.json");
...
function acquireChildResources(input) {
  ...
  try {
    ...
    if (input.fullResourceIsolation) {
      isolatedPiRoot = mkdtempSync3(path11.join(tmpdir6(), "ypi_isolated_pi_"));
      chmodSync3(isolatedPiRoot, 448);
      const isolatedAgentDir = path11.join(isolatedPiRoot, "agent");
    ...
    return {
      ...
      cleanup() {
        ...
        if (isolatedPiRoot)
          rmSync7(isolatedPiRoot, { recursive: true, force: true });
      }
    };
  } catch (error) {
    workspace?.cleanup();
...
async function runRecursiveChild(runtime, request) {
  ...
  try {
    ...
    if (resources.isolatedPiRoot) {
      env.PI_CODING_AGENT_DIR = path12.join(resources.isolatedPiRoot, "agent");
      env.PI_OFFLINE = "1";
    }
    ...
    const processResult = await runChildProcess({
      ...
      ...resources.workspace.childLaunchGate ? {
        launchGate: {
          launcherPath: path12.join(runtime.root, "scripts", "launch-implementer-child.ts"),
          ...resources.workspace.childLaunchGate
        }
      } : {}
    });
...
var moduleExtensionPath = path13.basename(modulePath) === "rlm_query.mjs" && path13.basename(path13.dirname(modulePath)) === "dist" ? path13.join(path13.dirname(modulePath), "..", "extensions", "recursive.ts") : fileURLToPath2(new URL("../recursive.ts", import.meta.url));
    ...
    if (flags.async) {
      ...
      try {
        ...
        await writeStdout(`${JSON.stringify({
          job_id: path13.basename(path13.dirname(job.jobPath)),
...
var invokedPath = process.argv[1] ? path13.resolve(process.argv[1]) : "";
